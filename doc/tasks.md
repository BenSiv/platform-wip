...
| 77 | Fix `entity_id` race condition in `ledger.lua`'s create path | **Live bug, independent of any DB migration.** `ledger.lua:61-79` assigns `entity_id` via `SELECT MAX(event_id)`, on the stated assumption of "single-process CLI, no concurrent writers" -- false under the actual concurrent CGI deployment; two simultaneous entity-creation requests can collide on the same id. Found during the MariaDB migration investigation (`doc/mariadb-migration.md`), but should be fixed on SQLite now (e.g. `BEGIN IMMEDIATE` to serialize writers, or a persistent connection + `last_insert_rowid()`), not deferred to the migration. |
| 78 | Add MariaDB C binding to luam (`lib/mariadb/lmariadb.c`) | MariaDB migration Phase 0 -- see `doc/mariadb-migration.md`. Cross-repo (luam, not platform-wip): new binding following `bcrypt.c`/`hmac.c`'s shape, linking `libmariadb-dev`. |
| 79 | Add persistent-connection MariaDB wrapper to luam (`lib/database.lua`-equivalent) | MariaDB migration Phase 1 -- see `doc/mariadb-migration.md`. Depends on #78. Persistent-per-process connection, not open-per-call -- overlaps with #57's persistent-process goal. |
| 80 | Swap `src/db.lua` to the new MariaDB binding | MariaDB migration Phase 2 -- see `doc/mariadb-migration.md`. Depends on #79. Should be the only platform-wip file whose internals change, per its own design intent. |
