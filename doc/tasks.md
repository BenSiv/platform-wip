...
| 82 | Stand up managed MariaDB and cut production over | MariaDB migration Phase 4 -- see `doc/mariadb-migration.md`. Depends on #81 (done). Keep the SQLite file + snapshot policy as a rollback path for one full deploy cycle. Also needs `libmariadb-dev` added to `elab/schema/platform/Dockerfile`'s builder stage -- the production binary is currently built without MariaDB support compiled in (gracefully, not an error) since that image doesn't have the header yet. If "managed" ends up meaning Cloud SQL specifically, that's the same GCP project-level CDPA as #95 -- no new agreement needed, just worth confirming #95 is settled before this cutover, not after. |
...
| 93 | Add a "reason for change" field to the ledger's update/create events | Found during #92's compliance research (see `~/software/docs/ai-agent-and-lab-data-compliance-research.md`). GLP's own audit-trail expectation is old value + new value + user + timestamp + **reason for the change** -- `ledger.append_update`/`append_create` capture everything except the reason. Not urgent (no stated GLP certification requirement today), but a real, concrete gap if one is ever pursued. |
...
| 95 | Confirm the Google Cloud Data Processing Addendum (CDPA) is accepted for this project | **Real, concrete, actionable gap found during #91's research** -- see `~/software/docs/ai-agent-and-lab-data-compliance-research.md`. Not automatic; requires a human with GCP Organization/Billing Account admin access to check/accept via Console → IAM & Admin → Privacy & Security → "Review and Accept". No `gcloud` CLI surface exists for this (confirmed) -- I can't check or accept it myself. Most concrete action item from the whole compliance research pass, since it's a real legal/contractual gap, not a design one. |
| 96 | Confirm/obtain a Google Cloud HIPAA BAA, if PHI is ever in scope | Same console location as #95, separate "Review and Accept" action. Contingent -- only actually needed if PHI-adjacent data could reach the chat agent or this deployment generally; worth a deliberate yes/no decision either way rather than leaving it unstated. |
| 88 | Design separation between science-analysis data and operational/audit data | **Ranked 5th.** From user notes: "raw data schema vs operational data schema separation" -- corrected: NOT about Benchling import data, but about the platform's own data. The distinction is between user-entered scientific data meant for analysis (entity/reagent/experiment/sample rows, etc.) and operational data whose purpose is auditing (the `entity_event` ledger, user/session/admin activity). Today both live in one undifferentiated schema/storage model -- worth designing whether they should be separated (storage location, access patterns, retention/backup policy) given their different purposes. |
| 87 | Persist full AI prompts/reasoning in the Knowledge Pool | **Unblocked -- #91/#92 research done, no showstopper found.** From user notes: "full ai thinking persistence in knowledge pool including prompts." Today `knowledge_retrieval` logs query text and hit metadata (score/tier/rank), not the full prompt sent to the model or its reasoning. The compliance research (`~/software/docs/ai-agent-and-lab-data-compliance-research.md`) didn't surface a blocking issue with storing this -- same audit-trail expectations (attributable/timestamped/complete) that already apply to the ledger would apply here too, nothing new. Ready to reprioritize whenever. |
...
| 90 | Reorganize templated deployment configs in `~/software` (housekeeping) | **Ranked 10th.** From user notes -- corrected: NOT an in-app UI, this is repo-level housekeeping. `~/software/elab/schema/platform/`'s templated configs (`schemas/`, `views/`, `extensions/`, `templates/`, `theme.json`, Dockerfile, `apache-platform.conf`, `startup-platform.sh.tpl`, etc.) are currently organized ad hoc; consolidate into one coherent, navigable structure in the software repo itself. Distinct from #89 (that's an in-app admin UI; this is a repo layout/housekeeping pass). |
