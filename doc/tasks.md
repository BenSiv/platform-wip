...
| 70 | Add deployment-configurable `system_prompt_extra` for the chat agent | **2nd.** No current way for a deployment to add use-case-specific instructions to the agent's system prompt; real prior art exists in the fossil-scm fork. |
| 93 | Add a "reason for change" field to the ledger's update/archive events | **3rd.** Found during #92's compliance research. GLP's own audit-trail expectation is old value + new value + user + timestamp + **reason for the change** -- `ledger.append_update`/`append_archive` (which already routes through `append_update` internally) capture everything except the reason. Scoped design (discussed 2026-07-20): (1) nullable `reason` column on `entity_event`, threaded as an optional param through `append_update`/`append_create`/`append_archive`; (2) **per-schema opt-in**, not a mandatory global prompt -- e.g. `require_reason_on_update`/`require_reason_on_archive` schema flags (default false), matching how `required`/`display` are already per-field schema settings, so trivial edits on low-stakes types aren't forced through the same friction as substantive ones; (3) **archive defaults toward requiring it more often than update** -- archiving is rarer, more consequential, and the reason tends to be genuinely meaningful ("contaminated," "duplicate entry") rather than a throwaway note, unlike routine field edits; (4) three distinct capture touchpoints need their own handling -- the web edit/archive form (a real optional/required text input), the chat agent's `entity.update`/`entity.archive` tools (default the reason to a short summary of the user's own chat message, which usually already states the "why," rather than asking twice), and any automated path like the Benchling sync (fixed default string, e.g. "Synced from Benchling" / "No longer present in Benchling sync" -- no human in the loop to prompt). |
