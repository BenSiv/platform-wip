...
| 93 | Add a "reason for change" field to the ledger's update/create events | **3rd.** Found during #92's compliance research. GLP's own audit-trail expectation is old value + new value + user + timestamp + **reason for the change** -- `ledger.append_update`/`append_create` capture everything except the reason. Cheap, contained change; not urgent (no stated GLP certification requirement today) but worth doing while it's fresh. |
